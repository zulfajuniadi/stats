...
if (!defined('PSI_CONFIG_FILE')) {
     ...
     */
    define('PSI_CONFIG_FILE', APP_ROOT.'/config.ini');
    ...
    if (!is_readable(PSI_CONFIG_FILE) || !($config = @parse_ini_file(PSI_CONFIG_FILE, true))) {
        if (defined('PSI_INTERNAL_XML') && PSI_INTERNAL_XML === true) {
            echo "ERROR: config.ini does not exist or is not readable by the webserver in the config directory";
            die();
        }
    } else {
    ...
    if (!defined('PSI_OS')) { //if not overloaded in config.ini
        /* get Linux code page */
        if (PHP_OS == 'Linux') {
            if (file_exists('/etc/sysconfig/i18n')) {
                ...
                if (file_exists('/system/build.prop')) { //Android
                    ...
                    if (!defined('PSI_MODE_POPEN')) { //if not overloaded in config.ini
            ...
            if (!(defined('PSI_SYSTEM_CODEPAGE') && defined('PSI_SYSTEM_LANG')) //also if both not overloaded in config.ini
               && $contents && ( preg_match('/^(LANG="?[^"\n]*"?)/m', $contents, $matches)
               || preg_match('/^RC_(LANG="?[^"\n]*"?)/m', $contents, $matches)
               || preg_match('/^export (LANG="?[^"\n]*"?)/m', $contents, $matches))) {
                if (!defined('PSI_SYSTEM_CODEPAGE') && @exec($matches[1].' locale -k LC_CTYPE 2>/dev/null', $lines)) { //if not overloaded in config.ini
                ...
                if (!defined('PSI_SYSTEM_LANG') && @exec($matches[1].' locale 2>/dev/null', $lines)) { //also if not overloaded in config.ini
                    foreach ($lines as $line) {
            ...
            if (!(defined('PSI_SYSTEM_CODEPAGE') && defined('PSI_SYSTEM_LANG')) //also if both not overloaded in config.ini
                ...
                foreach ($lines as $line) {
                    if (preg_match('/^"?([^\."]*)\.?([^"]*)/', $line, $matches2)) {
                        ...
                        if (!defined('PSI_SYSTEM_CODEPAGE') && isset($matches2[2]) && !is_null($matches2[2]) && (trim($matches2[2]) != "") ) { //also if not overloaded in config.ini
                            define('PSI_SYSTEM_CODEPAGE', $matches2[2]);
                        }

                        if (!defined('PSI_SYSTEM_LANG')) { //if not overloaded in config.ini
                            $lang = "";
                            if (is_readable(APP_ROOT.'/data/languages.ini') && ($langdata = @parse_ini_file(APP_ROOT.'/data/languages.ini', true))) {
                                if (isset($langdata['Linux']['_'.$matches2[1]])) {
                                    $lang = $langdata['Linux']['_'.$matches2[1]];
    ...
    if (!defined('PSI_SYSTEM_CODEPAGE')) { //if not overloaded in config.ini
    ...
    if (!defined('PSI_JSON_ISSUE')) { //if not overloaded in config.ini
